...
public class DFSService extends DFSManager implements IDFSService
{
	...
	*/
	public DistributedFile getFile( String fileName ) {
		...
		return database.getFile( Utils.getId( fileName ) );
	}
	...
	*/
	...
	public DistributedFile get( String fileName ) throws DFSException
	{
		...
		try{
			...
			LOGGER.error( "Operation GET not performed. Try again later." );
			//e.printStackTrace();
	...
	*/
	private int makeReconciliation( final List<RemoteFile> files )
	{
		int size = files.size();
		if(size == 1)
			return 0;
	...
	public boolean put( String fileName ) throws DFSException, IOException
	{
		...
		try{
			...
			LOGGER.error( "Operation PUT not performed. Try again later." );
			completed = false;
			if(session.isClosed())
				session.close();
		}
	...
	public boolean delete( String fileName ) throws IOException, DFSException
	{
		...
		try {
			...
			database.removeFile( fileName, file.getVersion() );
		}
		catch( IOException | SQLException e ) {
			LOGGER.error( "Operation DELETE not performed. Try again later." );
			completed = false;
			if(session.isClosed())
				session.close();
		}
